...
'
    ...
    'IMPORT CSV
        ...
        FileName = Application.GetOpenFilename("Text Files (*.csv),*.csv", , "Provide CSV File:")
        ' Open .txt delimited by comma (feature not implemented yet)
        ' FileName = Application.GetOpenFilename("CSV Files (*.csv),*.csv,Text Files (*.txt),*.txt", , "Provide Text or CSV File:")
        ' But if FileName is a .txt will need to transform text to columns before continue
    ...
    'ADD VLOOKUP TO MAIN SHEET
        ...
        ExcelSheet2.Cells(1, 4).Formula = "=VLookup($A1, CSV!$A:$E, 3, False)"
        ExcelSheet2.Cells(1, 5).Formula = "=VLookup($A1, CSV!$A:$E, 4, False)"
        ExcelSheet2.Cells(1, 6).Formula = "=VLookup($A1, CSV!$A:$E, 5, False)"
        ExcelSheet2.Cells(1, 7).Formula = "=VLookup($A1, CSV!$A:$E, 2, False)"
